...
- **대화형 업무 조작**: 제목 기반 완료·메모 추가·검색·답장 초안, 모호한 대상은 후보 선택
- **작은 대화창**: 웹 미니카드와 선택적 Windows 보조 앱, 연결 조건에 따른 본체 최소화·복원
...
- 출퇴근 API는 설정된 TAGO 정류소의 버스 도착정보를 조회합니다. 키·정류소 누락 또는 실패 시 안내하며, 자차 소요시간을 임의 생성하지 않습니다. 실제 키·기기 검증은 별도입니다.
...
현재 구현·자동 검증·운영 검증의 구분은 [기술 레퍼런스](./doc/01-as-built-reference.md), 최근 기능은 [웹 미니카드·데스크톱](./doc/20-desktop-barista-mini-card.md)과 [대화형 업무 조작](./doc/21-conversational-task-actions.md)을 참고하세요.
