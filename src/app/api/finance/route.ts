...
export async function GET(request: NextRequest) {
  ...
  if (!forceRefresh && cache && now - cache.timestamp < CACHE_TTL_MS) {
    return NextResponse.json<FinanceApiResponse>({
      success: true,
      finance: cache.data,
      cached: true,
  ...
  if (exchangeResult.status === "fulfilled" && exchangeResult.value) {
    ...
    warnings.push(exchangeResult.reason instanceof Error ? exchangeResult.reason.message : "환율 조회 실패");
  }
  ...
  if (interestResult.status === "fulfilled" && interestResult.value) {
    ...
    warnings.push(interestResult.reason instanceof Error ? interestResult.reason.message : "금리 조회 실패");
  }
