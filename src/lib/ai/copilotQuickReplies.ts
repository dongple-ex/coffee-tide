...
export function getQuickReplies({
  ...
  // 1. 업무 상태별 생산성 액션 칩 (Productivity Action)
  if (hasUrgentTasks) {
    chips.push({
      id: "urgent_focus",
      icon: "🚨",
      label: "긴급 마감 건 해결법 알려줘",
    ...
    chips.push({
      id: "today_summary",
      icon: "☀️",
      label: "오늘 브리핑 3줄 요약해줘",
      query: "오늘 전체 일정과 꼭 챙겨야 할 핵심 사항을 3줄로 알기 쉽게 요약해줘.",
      category: "productivity",
    });
  }

  // 2. 캐릭터별 맞춤형 티키타카 롤플레잉 칩 (Roleplay & Rapport)
  switch (presetId) {
    case "karina":
      chips.push({
        id: "karina_talk",
        icon: "✨",
        label: "*라떼를 한 모금 마시며* 오늘 파이팅!",
  ...
  // 3. 시간대별 리프레시 / 회고 칩 (Refresh & Wellness)
  if (hour >= 17) {
    chips.push({
      id: "wrapup_handoff",
      icon: "🚪",
      label: "오늘 완료한 일과 내일 할 일 정리해줘",
    ...
    chips.push({
      id: "focus_timer",
      icon: "⏱️",
      label: "25분 뽀모도로 집중 가이드 해줘",
      query: "지금부터 25분 동안 완벽히 몰입해서 끝낼 수 있도록 핵심 태스크를 지정해 주고 집중 시작을 선언해줘!",
      category: "productivity",
    });
  }

  // 4. AI 캔버스 문서화 지원 칩 (Canvas Action)
  if (canvasEnabled) {
    chips.push({
      id: "canvas_action",
      icon: "🖌️",
      label: "회의록/메모 표 형태로 구조화해줘",
