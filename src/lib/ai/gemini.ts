...
const classifyCache = new Map<string, { category: UnifiedCategory; actionDirective: string }>();
...
function contentHash(item: UnifiedData): string {
  return createHash("sha1").update(`${item.id}|${item.title}|${item.content}`).digest("hex");
}
...
const CLASSIFY_SYSTEM = `역할: 수신된 업무 데이터를 분석하여 가장 알맞은 행동 지침 카테고리로 분류하는 분류기(Classifier)입니다.
...
export async function classifyTasks(
  ...
  try {
    ...
    const parsed = parseJsonLoose<
      { id: string; category: string; actionDirective: string }[]
    >(raw);
...
export async function askCopilot(
  ...
  const system = `역할: 사용자의 개인 AI 업무 비서입니다. 제공된 업무 데이터 컨텍스트를 기반으로 사용자가 오늘 해야 할 일과 우선순위를 브리핑합니다.
...
1. 오늘 처리할 최우선 업무 (중요도순 요약 나열)
2. 행동 지침 요약 및 우선순위 제안
3. 예상 소요 시간
4. 잠재적 위험 요소 (마감 임박, 긴급 장애 등)

어조: 사용자의 친근한 개인 비서입니다. 따뜻하고 가벼운 유머를 섞어 "~해드릴게요", "~해뒀어요" 같은 말투를 쓰되, 업무 내용·날짜·출처는 정확하고 명료하게 전달하십시오. 과장된 아부나 이모지 남발은 피하세요.`;
