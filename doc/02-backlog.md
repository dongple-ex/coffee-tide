...
> **현재 상태 대조: 2026-09-23 / `ea4bcb5` / 앱 1.2.2.** 기존 완료 표시는 당시 이력이다. 최신 기능의 남은 검증·정책 검토는 M 항목을 우선 확인한다.

## 작업 규약 (먼저 읽기)

- **코드 변경 검증 3종 세트**: `npx tsc --noEmit -p tsconfig.json` · `npm run lint` · `npm run build` — 셋 다 통과해야 함.
- **문서만 변경한 경우**: 링크·파일/심볼 참조·버전 정합성과 `git diff --check`를 확인한다. 기능 테스트를 실행했다면 범위·날짜·결과를 별도로 기록하며 코드 전체 검증으로 확대 해석하지 않는다.
...
| ID | 제목 | 상태/우선순위 | 영역 |
| :-- | :-- | :-- | :-- |
| **M1** | 대화형 업무 조작의 실제 연동·브라우저 검증 | P1, 자동 테스트 통과 / 외부 검증 대기 | 검증 |
| **M2** | 데스크톱 origin 허용 정책·최신 배포 패키지 검증 | P1 | 정책/배포 |
| **M3** | 미니카드·왼쪽 Shift·실제 창 최소화/복원 | P1, 자동 테스트 통과 / 실기기 별도 | 검증 |
| **M4** | AI 작업 복귀·실기기 푸시·아카이브 RLS | P1 | 운영 검증 |
...
> 근거: [`00-product-spec.md`](./00-product-spec.md)의 "핵심 제품 원칙"과 "현재 구현과의 차이". 아래는 무연동 우선 흐름을 도입하던 당시의 격차 기록이다. 현재 manual/paste와 무연동 대화 경로는 구현되어 있으며, 각 항목의 완료 상태를 따른다.
...
### L1. 로컬 모델 공급자 확장 — 브라우저 경로 구현 / 서버형 공급자 P1

- **현재**: `FallbackEngine`은 정규식·템플릿이며 실제 LLM이 아니다. LLM 산출물 연동도 파일 스캔 기능이다.
- **2026-09-23 대조**: Chrome Prompt API 경로와 준비 상태·최근 대화 문맥은 구현됐다. `classifyTasks()`는 계속 규칙 분류이며, Ollama/LM Studio 등의 서버형 공급자 통합과 모델 기반 분류는 별도 과제다. 브라우저 모델 가용성을 모든 기기의 지원으로 안내하지 않는다.
...
- **현재**: 서버 Copilot은 최대 80개 항목과 최근 8턴(턴당 최대 500자)을 받는다. 정책 기반 지식 검색과 완료 문서 아카이브 검색이 연결되어 있다. 이전의 ‘대화 문맥·RAG 없음’ 설명은 더 이상 현재 상태가 아니다.
...
- **남은 문제**: 완료 캔버스 아카이브의 내용 해시·청크·질문별 검색은 구현됐다. 이를 모든 로컬 폴더의 증분 임베딩 색인으로 확대 해석하지 않는다. 전체 파일 변경 감지·HWPX·EML·OCR 등의 범위는 구현 전 재점검한다.
...
- 구현: 후보 번호 범위·사용자/채널별 5분 수명, 부정 문형 방어, 메모 추가, 외부 완료 응답 대기, 답장 작성 지시 분리.
- 근거: [21 문서](./21-conversational-task-actions.md), 관련 웹 테스트 39개 통과(창 제어 4개 포함, 2026-09-23).
- 남은 확인: 실제 Notion·Obsidian 완료, Gmail 초안 텍스트, Outlook 생성·임시보관함 저장/실패, 여러 대화창의 후보 전환을 브라우저에서 확인한다.
- 완료 기준: 화면 메시지·앱 상태·외부 원본이 일치하며, 예시·부분 실패를 외부 저장 성공으로 표시하지 않는다.

### M2. 데스크톱 브리지 정책과 배포본 — P1

- 현재: 설정 origin 외 localhost/127.0.0.1·`.vercel.app` 전체·공식 도메인을 허용한다. 코드/토큰 페어링은 유지되지만 origin 하나만 신뢰하는 정책은 아니다.
- 제안: 필요한 배포 origin 목록과 연결 토큰의 origin 결합 여부를 검토하고, 정책 결정 후 회귀 테스트를 추가한다.
- 배포: 웹 다운로드 ZIP과 최신 소스의 일치·서명·실행 허용 여부를 별도 확인한다. 소스 테스트 통과만으로 배포 파일이 최신이라고 간주하지 않는다.
- 완료 기준: 명시된 신뢰 범위 밖 요청이 거부되고, 배포본의 버전·해시·패키징 결과와 실제 실행 검증이 남아 있다.

### M3. 웹 미니카드·Windows 창 제어 — P1

- 자동 검증: 데스크톱 테스트 17개와 웹 창 제어 계약 테스트 통과. [20 문서](./20-desktop-barista-mini-card.md) 참조.
- 남은 확인: Chrome/Edge 실창·다중 창·다중 모니터·IME·왼쪽/오른쪽 Shift 구별·연결 해제·강제 종료·입력 보존을 실제 기기에서 확인한다.
- 완료 기준: 미연결 상태에는 본체 최소화 없음, 연결된 정확한 창만 최소화·복원, 복원 시 PiP 닫힘, 브라우저 제약 발생 시 사용자가 복구 가능.

### M4. 서버 AI 결과·아카이브·푸시 운영 검증 — P1

- 구현: 결과 저장 후 일반 알림, 작업 소유자 검증, 복귀 복원, 문서 아카이브 검색·정책 필터.
- 남은 확인: 운영 Redis/VAPID·Supabase 마이그레이션/RLS·다른 계정 접근 거부·iOS/Android 알림·잠금 후 복원.
- 완료 기준: 실제 계정·기기·배포 버전을 명시한 증거가 있고, 강제 종료된 작업의 재실행과 단순 결과 복원을 구분한다.

---

_최종 갱신: 2026-09-23. 자동 테스트 통과, 실계정 검증, 원격 배포는 서로 다른 상태로 기록한다._
