...
1. **플레이어 제어 및 레이아웃**:
   ...
   - 뷰어 모드:
     ...
     - **집중 축소 모드**: CoffeeTide 본문은 반투명 블러 배경으로 보이게 유지하고, 우측 하단에 정책 최소 크기 이상의 영상 플레이어와 CoffeeTide 로고·복원 버튼을 표시. 축소 플레이어는 자막이 영상 본문을 덜 가리도록 하단 레터박스 안전 영역을 확보하며, 동일 iframe을 재사용해 축소·복원 시 재생 위치를 보존.
