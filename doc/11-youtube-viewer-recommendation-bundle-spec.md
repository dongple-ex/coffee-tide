...
1. **플레이어 제어 및 레이아웃**:
   ...
   - 뷰어 모드:
     ...
     - **집중 축소 모드**: CoffeeTide 본문은 반투명 블러 배경으로 보이게 유지하고, 우측 하단에 정책 최소 크기 이상의 영상 플레이어와 CoffeeTide 로고·복원 버튼을 표시. 모바일에서는 영상을 선택하면 비차단형 축소 플레이어로 시작해 배경 업무 화면의 스크롤과 조작을 허용하고, 확대할 때만 전체 모달로 전환한다. 상단 제어는 아이콘만 노출하고 기능명은 마우스 오버·키보드 포커스 툴팁으로 제공한다. 축소 플레이어는 자막이 영상 본문을 덜 가리도록 하단 레터박스 안전 영역을 확보하며, 동일 iframe을 재사용해 축소·복원 시 재생 위치를 보존.
