...
> ⚠️ **역사 문서 (Phase 1 · TimePilot 시절)** — 초기 Microsoft 단일 채널 설계 기록으로, 현재 coffeeTide 정본과 다릅니다. 특히 본문의 `/api/auth/signin`(MS OAuth 시작점)·`/api/auth/callback`(콜백) 설명은 현행 설계와 **정반대**입니다: 현행은 `signin`=게스트 세션 발급, Outlook OAuth=`/api/auth/outlook` → `/api/auth/outlook/callback`. 신규 작업 기준: [`00-product-spec.md`](../00-product-spec.md) 및 [`01-as-built-reference.md`](../01-as-built-reference.md).
