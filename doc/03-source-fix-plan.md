...
> **2026-09-23 안내:** 아래는 7월 점검·수정 이력입니다. 현재 전체 검증 상태나 미해결 목록은 [구현 현황](./01-as-built-reference.md)과 [백로그](./02-backlog.md)를 우선합니다.

## 0. 당시 검증 baseline (2026-07-26 측정)
...
`02-backlog.md` §작업 규약은 "검증 3종 세트 셋 다 통과"를 요구하므로, **당시 main은 규약 위반 상태**였습니다. K5는 아래 이력대로 2026-07-27 해소되었습니다. 이 과거 실패를 현재 main의 검사 결과로 인용하지 않습니다.
...
| **K2** | `/api/commute` 전면 하드코딩 데이터를 실시간으로 표시 → **공공데이터포털 실연동** | **P0** | 제품 신뢰 | ✅ 2026-07-27 코드 구현 (커밋 `177090a`) — 실키 E2E 검증만 대기 (`DATA_GO_KR_SERVICE_KEY` 미설정) |
