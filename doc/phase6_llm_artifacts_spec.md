...
사용자는 로컬 LLM 도구(**Claude Code, Gemini** 등)가 남긴 **작업 산출물 파일**(예: `MEMORY.md`, 세션 메모, 에이전트 출력)을 coffeTide에서 읽어 **"오늘 처리한 내용"** 을 한눈에 보고 싶다. 또한 그 내용을 **사람이 읽기 좋게 Obsidian**에서 탐색하고 싶다.
...
- (Obsidian) "오늘 요약 내보내기"를 누르면 볼트에 `coffeTide_LLM/2026-07-04.md`가 생성/갱신되고, Obsidian에서 원본 파일로의 링크와 함께 읽을 수 있다.
...
- 위치: `<vault>/coffeTide_LLM/YYYY-MM-DD.md` (하위 폴더는 설정 가능).
...
- "Obsidian에 오늘 요약 내보내기" → 볼트에 `coffeTide_LLM/YYYY-MM-DD.md`가 생성되고, Obsidian에서 백링크로 원본을 연다.
...
- **Q4** ✅ 결정: **자동 미러링**. `/api/mails` 동기화 시 오늘 항목을 볼트 `coffeTide_LLM/YYYY-MM-DD.md`에 idempotent upsert(내용 동일 시 미기록).
