...
- 대시보드 준비 후 활성 사용자에 한해 자동 브리핑을 조회한다. 결과가 없으면 AI 바리스타 영역 안에서 최근 24시간 수신 내역이 없음을 안내한다.
- AI 바리스타 제목에는 별도 Spark 활성 배지를 표시하지 않아 중복 시그널을 줄인다.
- 자동 카드는 설정이 활성화된 경우에만 기존 질의응답보다 위에 렌더링하고 `⚡ Gemini Spark 24시간 자율 비서` 배지를 표시한다. 카드 내부에는 별도의 `활성` 상태 배지를 중복 표시하지 않는다.
- Gemini Spark는 빠른 위젯 도구함에 노출하지 않으며, 별도 위젯 패널 없이 AI 바리스타의 보조 AI 채널로만 동작한다.
