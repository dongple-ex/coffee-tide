...
coffeTide는 여러 채널의 업무 데이터를 하나의 대시보드로 통합하고, AI로 트리아지하며, 자동화 규칙으로 정리하는 Next.js 16 앱입니다. **무연동 우선**: manual/paste가 1급 소스이며 외부 연동 없이 전 기능이 동작합니다.

| 구분 | 내용 |
| :--- | :--- |
| 채널 | **manual·paste(1급)**, Outlook, Gmail, Notion, Obsidian, 로컬 문서, LLM 산출물 |
| 로그인 | 게스트 세션(`coffeTide 시작하기`) + 서비스별 개별 연동 |
...
- **로그인 흐름**: `/api/auth/signin`이 **게스트 세션**(`guest@coffetide.dongple.kr`) 발급 → 대시보드 진입 후 서비스별 개별 연동.
- **서비스 연동**:
...
| `/api/tasks/llm-digest` | POST | 오늘 LLM 산출물 → Obsidian `coffeTide_LLM/YYYY-MM-DD.md` 수동 내보내기 |
...
- **LLM 산출물 (phase6)**: 폴더 스캔(`LlmArtifactAdapter`, frontmatter 파싱·발췌 500자) + Obsidian 연동 시 동기화마다 오늘 항목을 `coffeTide_LLM/YYYY-MM-DD.md`에 idempotent upsert(Q4=자동).
...
- OAuth 리다이렉트 URI: 로컬 `http://localhost:3000/api/auth/...`, 배포 `https://coffeTide.dongple.kr/api/auth/...`.
