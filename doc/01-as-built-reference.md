...
- **Cloud Tool Registry**: 소스 코드에 정적으로 등록된 TypeScript 함수만 Vercel에서 실행한다. 조회 도구는 현재 화면 업무 집계(`workspace.task_summary`)와 한국은행 환율·금리(`finance.market_snapshot`)이며, Phase C 초안 도구는 일정(`calendar.event_draft`)·메일 답장(`email.reply_draft`)·보고서(`document.report_draft`)다. 사용자 인증, 추가 인자 거부, 문자열·열거값 검사, 사용자·도구별 분당 20회 제한, 실행 시간·출력 크기 제한과 식별자 해시 로그를 적용한다. AI 바리스타는 자연어 질문에 필요한 도구 하나를 Gemini function calling으로 제안받아 Registry에서 실행한다. 초안 도구는 외부 저장·발송 없이 구조화된 결과만 반환하고 모바일 검토 카드에서 수정·복사·취소한다. 미등록 함수·인자와 병렬·반복 호출은 거부하며, 재요약 실패 시 검증된 도구 요약으로 폴백한다. 상세 설계는 [`11-cloud-tool-registry-plan.md`](./11-cloud-tool-registry-plan.md) 참조.
