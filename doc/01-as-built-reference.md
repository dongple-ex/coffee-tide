...
- **Cloud Tool Registry**: 소스 코드에 정적으로 등록된 TypeScript 함수만 Vercel에서 실행한다. 조회 도구는 업무 집계(`workspace.task_summary`)와 한국은행 환율·금리(`finance.market_snapshot`), 초안은 일정(`calendar.event_draft`)·메일 답장(`email.reply_draft`)·보고서(`document.report_draft`), 승인형 외부 쓰기는 Calendar(`calendar.event_create`)·Drive(`drive.report_save`)다. Gemini 자동 함수 선택은 읽기·초안 하나로 제한한다. 외부 쓰기는 서버 미리보기 → 세션·입력 해시 결합 5분짜리 1회 승인 → 멱등 실행 순서만 허용하며, Supabase service-role 전용 승인·멱등성·감사·분산 호출 제한 저장소를 사용한다. 원문 입력과 OAuth 토큰은 저장하지 않고 삭제·권한 변경·메일 발송 도구는 등록하지 않는다. 상세 설계는 [`11-cloud-tool-registry-plan.md`](./11-cloud-tool-registry-plan.md) 참조.
